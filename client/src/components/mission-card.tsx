...
export default function MissionCard({ mission, role = "worker" }: MissionCardProps) {
  const category = CATEGORIES.find(c => c.id === mission.category) || CATEGORIES[0];
  const CategoryIcon = category.icon as any;
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground line-clamp-1">{mission.client.name}</p>
            </div>
          </div>
